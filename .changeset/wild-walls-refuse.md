...
Contract corrections. These are the changes that are cheap now and a
...
capability gating it. The CLI previously *deleted* type-only edges during its scan, which made
...
Edges that dedupe onto one another now *merge* their attributes by intersection rather than
